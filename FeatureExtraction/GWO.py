...
from FeatureExtraction.GWO_helper import jfs
from Utils.Constants import FINAL_DATASET_PATH_GWO, PREPROCESSED_DATA_PATH_FS, SAVE_GWO_CHANNELS_PATH
...
def use_gwo(participant_list: [int] = range(1, 10), classify_type: str = "Arousal", n_particle: int = 128,
    ...
    T = n_iterations
    print(
        f"Run GWO channel selection method with {CLASSIFY_TYPE} and with {N} particles and with {T} max iterations on {n_cores} cores")
    ...
    save_path_data_training = Path(FINAL_DATASET_PATH_GWO, "data_training.npy")
    save_path_label_training = Path(FINAL_DATASET_PATH_GWO, "label_training.npy")
    save_path_data_testing = Path(FINAL_DATASET_PATH_GWO, "data_testing.npy")
    save_path_label_testing = Path(FINAL_DATASET_PATH_GWO, "label_testing.npy")
    FINAL_DATASET_PATH_GWO.mkdir(exist_ok=True)
    SAVE_GWO_CHANNELS_PATH.mkdir(exist_ok=True)
    with ProcessPoolExecutor(max_workers=n_cores) as executor:
        for label, zx, p, ch, acc in executor.map(exec_gwo, participant_list):
    ...
    pd.DataFrame(backup, columns=["participant", "channels", "n_channels", "channel_acc"], index=None).to_csv(
        Path(SAVE_GWO_CHANNELS_PATH, f"gwo_channels_{classify_type}.csv"), index=False)
...
if __name__ == '__main__':
    ...
    use_gwo(participant_list=range(1, 10), classify_type="Arousal", n_particle=2, n_iterations=1, components=20,
            n_cores=1)
